...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 2. Dispatch credentials packet payload down to Port 5000 Express login path
      ...
      if (err.response && err.response.status === 401 || err.response?.status === 404) {
        ...
        setError('Failed to connect to the authentication server. Ensure node index.js is active.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-[calc(100vh-80px)] w-full flex items-center justify-center p-4">
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-12 glass-card rounded-3xl overflow-hidden shadow-2xl border border-slate-800">
        ...
        <div className="md:col-span-6 p-8 sm:p-12 flex flex-col justify-center bg-slate-950/40">
          <div className="mb-6 text-center md:text-left">
            ...
            <p className="text-slate-400 text-xs mt-1.5 uppercase font-bold tracking-wider">
              Log into the learning hub network
            </p>
          </div>
        ...
        <div className="hidden md:block md:col-span-6 relative min-h-[500px]">
          <img src={authSideImage} alt="Cover Graphic" className="absolute inset-0 w-full h-full object-cover select-none" />
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && (err.response.status === 401 || err.response?.status === 404)) {
        ...
        const activeEndpoint = import.meta.env.VITE_API_URL ? "production cloud engine" : "local server processes";
        setError(`Failed to connect to the authentication server. Ensure the ${activeEndpoint} is running.`);
      }
    } finally {
      setLoading(false);
    }
