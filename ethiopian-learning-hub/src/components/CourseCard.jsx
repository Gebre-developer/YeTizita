...
const CourseCard = ({ course }) => {
  ...
  // ✅ SAFELY EXTRACT API BASE URL FOR STATIC UPLOAD PATH RESOLUTION
  const API_BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:5000";

  // ✅ SAFELY APPEND LOCAL HOST ADDRESS TO SECURE DISK MULTIPART UPLOADS IF APPLICABLE
  const fileDownloadUrl = course.fileUrl?.startsWith('http') 
    ? course.fileUrl 
    : `${API_BASE_URL}${course.fileUrl}`;
  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden border border-slate-800 hover:border-slate-700 transition-all duration-300 flex flex-col h-full shadow-lg group bg-slate-900/40">
      ...
      <div className="h-44 bg-slate-950 relative overflow-hidden">
        <img 
          // ✅ FIXED: Replaced webpage string fallback with a valid placeholder asset path link
      ...
      <div className="p-5 flex flex-col flex-grow space-y-3">
        <h3 className="font-bold text-base text-white line-clamp-1 group-hover:text-amber-400 transition duration-300">
          {course.title}
        </h3>
        ...
        <div className="pt-2 border-t border-slate-800/60">
          {course.isEnrolled ? (
            course.fileUrl ? (
              <a 
                // ✅ FIXED: Points safely to full asset path mapping parameters
        ...
        <div className="flex items-center justify-between pt-2">
          <span className="text-[11px] text-slate-500 font-medium">
            ...
            <span className="text-slate-400">
              {/* ✅ FIXED: Accesses the username property inside the include object securely */}
              {course.instructor?.username || course.instructor || 'Unknown Instructor'}
            </span>
          </span>
          <Link 
